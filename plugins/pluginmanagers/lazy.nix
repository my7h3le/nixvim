...
lib.nixvim.neovim-plugin.mkNeovimPlugin {
  ...
  extraOptions =
    let
      ...
      lazyPluginType =
        with types;
        types.coercedTo lazyPluginSourceType
          (source: {
            inherit source;
            __unkeyed = lib.mkIf (isGitURL source) "${source}";
            name = if lib.isDerivation source then lib.getName source else builtins.baseNameOf source;
            dir = lib.mkIf (lib.isDerivation source || lib.isPath source) "${source}";
            url = lib.mkIf (isGitURL source) "${source}";
          })
          (
            submodule (
              { config, ... }:
