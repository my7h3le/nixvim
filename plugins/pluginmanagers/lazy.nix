...
nixvim.neovim-plugin.mkNeovimPlugin {
  ...
  extraOptions =
    let
      lazyPluginType = types.coercedTo types.package (pkg: { inherit pkg; }) (
        types.submodule (
          { config, ... }:
          ...
          with types;
          {
            ...
            options = {
              ...
              dependencies = helpers.mkNullOrOption (types.either (types.listOf lazyPluginType) (types.listOf package)) "Plugin dependencies";
