...
nixvim.neovim-plugin.mkNeovimPlugin {
  ...
  extraOptions =
    let
      lazyPluginSubmoduleType = types.submodule (
        { config, ... }:
        ...
        with types;
        {
          ...
          options = {
            ...
            dependencies = helpers.mkNullOrOption (lazyPluginsListType) "Plugin dependencies";
      ...
      lazyPluginsListType = types.coercedTo (types.listOf types.package) (
        packages: map (pkg: if lib.isDerivation pkg then { inherit pkg; } else pkg) packages
      ) (types.listOf lazyPluginSubmoduleType);
    in
    {
