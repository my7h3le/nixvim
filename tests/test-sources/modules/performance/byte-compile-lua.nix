  ...
  default.module =
    { config, ... }:
    {
      ...
      extraFiles = {
        ...
        # Save nixvim-print-init path to a file because it can't be used in init.lua due to infinite recursion
        "nixvim-print-init-path".text = "${config.printInitPackage}/bin/nixvim-print-init";
      };
      ...
      extraConfigLuaPost = ''
        ...
        local nixvim_path = vim.fn.readfile(
          vim.api.nvim_get_runtime_file("nixvim-print-init-path", false)[1]
        )[1]
        local init_content = vim.fn.system(nixvim_path)
